...
{
    [ShopAuthorize]
    public class SuKienKhuyenMaiController : BaseController
    {
        ...
        public SuKienKhuyenMaiController(IDetailMenuRepository detailMenuRepository, IUnitOfWork unitOfWork, HttpContextBase httpContext, ITagTinTucRepository tagTinTucRepository, ISalePageRepository salePageRepository)
        {
        ...
        public ActionResult Index(int? page)
        {
            ...
            IList<DetailMenu> detail = _detailMenuRepository.GetMany(o => o.id_Menu == 20 && o.ShowMenu == true && o.TinhTrangSP == true && o.ShowKhuyenMai == false).OrderByDescending(o => o.sDate).ToList();
        ...
        public ActionResult Addnew(ThemTinTucModel model)
        {
            ...
            //{
                #region them moi su kien khuyen mai
        ...
        {
            #region Update su kien khuyen mai
        ...
        [HttpPost]
        public JsonResult UpdateShowTinKM(int idtintuc, bool Checked)
        {
            if (Checked)
            {
                using (var context = new ShopDataContex())
                {
                    context.Database.ExecuteSqlCommand("update DetailMenu set ShowTinKhuyenMai='True' where id_ ={0}", idtintuc);
            ...
            else
            {
                using (var context = new ShopDataContex())
                {
                    context.Database.ExecuteSqlCommand("update DetailMenu set ShowTinKhuyenMai='False' where id_ ={0}", idtintuc);
        ...
        //  lay danh sach tag có sẵn để load lên
        [HttpGet]
        public JsonResult getListTag()
        {
            IList<TagName> _Tag = _detailMenuRepository.GetTagName();
