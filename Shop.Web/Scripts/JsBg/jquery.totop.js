(function ( $ ) {
	$.fn.totopscroller = function(options) {
		...
		var defaults = {
			...
			link: false,
			linkTarget: '_self',
			toTopHtml: '<a href="#" class="back-to-top"><i aria-hidden="true" class="fa fa-angle-up"></i></a>',
			toBottomHtml: '<a href="#" class="back-to-top"><i aria-hidden="true" class="fa fa-angle-down"></i></a>',
		...
		var init = function() {
			...
			if (settings.showToBottom)
			{
				b_bottom.click(function(e) {
					...
					if ($('#single-box-height-scroll').length == 1) {
					    $('html, body').animate({ scrollTop: $('#single-box-height-scroll').position().top }, 'slow');
				    } else {
